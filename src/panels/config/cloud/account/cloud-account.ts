...
// NOTE (design exploration): this card lets PR reviewers preview every UI
// state without needing the matching real cloud state. It overrides the
// effective state the page renders from and must be removed before shipping.
interface CloudPreview {
  enabled: boolean;
  account: "subscribed" | "trial" | "expired" | "signed_out";
  onboarded: boolean;
  connection: "connected" | "connecting" | "disconnected";
  remote: "connected" | "disabled";
  backup: "fresh" | "stale" | "failed" | "none";
  alexa: boolean;
  google: boolean;
  webrtc: boolean;
  webhooks: boolean;
...
export class CloudAccount extends SubscribeMixin(LitElement) {
  ...
  @state() private _preview: CloudPreview = {
    enabled: false,
    account: "subscribed",
    onboarded: true,
  ...
  protected render() {
    ...
    const onboarding = !signedOut && !this._onboarded;
    ...
    return html`
      <ha-card outlined>
        <div class="account-header">
          <span class="card-title">Nabu Casa account</span>
          <img
            class="nc-logo"
            alt="Nabu Casa"
        ...
        <div class="card-content">
          <ha-md-list>
            ...
            <ha-md-list-item>
              ...
              <span slot="supporting-text">${this._renewalDate}</span>
            </ha-md-list-item>
            <ha-md-list-item>
              <span slot="headline">Subscription</span>
              <span slot="supporting-text">${this._subscriptionPlan}</span>
            </ha-md-list-item>
          </ha-md-list>
        </div>
    ...
    return html`
      <ha-card outlined>
        ...
        <div class="extras-content">
          <p>
            The best of Home Assistant, while funding its development. Securely
            reach your home from anywhere, add voice assistants, keep an
            off-site backup, and more. Built by the team behind Home Assistant.
          </p>
          <ha-md-list>
            ${this._featureRow(
              mdiEarth,
              "Remote access",
              html`<span class="status-line"
                ><span
                  class="status-dot ${this._remoteEnabled
                    ? "enabled"
                    : "disabled"}"
                ></span
                >${this._remoteEnabled ? "On" : "Off"}</span
              >`,
              "/config/cloud/remote"
            )}
            ${this._featureRow(
            ...
            ${this._featureRow(
              mdiMicrophoneMessage,
              "Alexa & Google",
              this._alexaLinked && this._googleLinked
                ? "Alexa and Google linked"
                : this._alexaLinked
                  ? "Alexa linked"
                  : this._googleLinked
                    ? "Google linked"
                    : "Control your home with Amazon Alexa and Google Assistant",
            ...
            ${this._featureRow(
              ...
              html`<span class="status-line"
                ><span
                  class="status-dot ${this._webrtcEnabled
                    ? "enabled"
                    : "disabled"}"
                ></span
                >${this._webrtcEnabled ? "On" : "Off"}</span
              >`,
              "/config/cloud/webrtc"
            )}
            ${this._featureRow(
  ...
  private _renderSignedOut(): TemplateResult {
    const features: [string, string, string][] = [
      [
        mdiEarth,
        "Access from anywhere",
        "Securely reach your Home Assistant from any device, with no port forwarding or VPN.",
      ],
      [
        mdiMicrophoneMessage,
        "Voice control",
        "Use Home Assistant with Amazon Alexa and Google Assistant.",
      ],
      [
        mdiBackupRestore,
        "Backups kept safe",
        "Your latest backup, stored privately and encrypted, so you can restore your whole system on first boot.",
      ],
      [
        mdiMicrophone,
        "Fast, accurate voice",
        "More languages and higher accuracy for Home Assistant's privacy-first voice assistant.",
      ],
      [
        mdiVideo,
        "Smoother streaming",
        "Better camera feeds and Music Assistant playback, wherever you are.",
      ],
      [
        mdiWebhook,
        "Webhooks for automations",
        "Trigger Home Assistant from external services securely, without exposing your instance.",
      ],
      [
        mdiFaceAgent,
        "Support from real humans",
        "DIY is great until you hit a brick wall. Our team is here to help you get set up.",
      ],
    ];
    ...
    return html`
      <ha-card outlined>
        ...
        <div class="card-actions split">
          <ha-button appearance="filled" @click=${this._openRegister}>
            Start your free trial
          </ha-button>
          <ha-button appearance="plain" @click=${this._openLogin}>
            Sign in
          </ha-button>
        </div>
      </ha-card>
      ...
      <ha-card outlined>
        <div class="card-content">
          ...
          <ha-md-list>
            ${features.map(
              ([icon, headline, text]) => html`
                <ha-md-list-item>
                  <ha-svg-icon slot="start" .path=${icon}></ha-svg-icon>
                  <span slot="headline">${headline}</span>
                  <span slot="supporting-text">${text}</span>
                </ha-md-list-item>
              `
            )}
          </ha-md-list>
      ...
      <ha-card outlined>
        <div class="card-content">
          <div class="feature-header">
            <ha-svg-icon .path=${mdiHeart}></ha-svg-icon>
            <span class="card-title">More than a subscription</span>
          </div>
          <p class="muted">
            Every subscription funds Home Assistant and the Open Home
            Foundation, paying full-time developers and keeping the project
  ...
  private _renderPreviewCard(): TemplateResult {
    return html`
      <ha-card outlined class="preview-card">
        <div class="card-content">
          <div class="feature-header">
            <ha-svg-icon .path=${mdiFlaskOutline}></ha-svg-icon>
            <span class="card-title">Preview controls</span>
            <ha-switch
              class="header-switch"
          ...
          <ha-alert alert-type="info">
            For reviewing this pull request only. These controls override what
            is shown above without touching your real cloud settings, and should
            be removed before shipping.
          </ha-alert>
          ${this._preview.enabled
            ? html`
                <div class="preview-controls">
                  ${this._previewSegment("Account", "account", [
                    ["subscribed", "Subscribed"],
                    ["trial", "Trial"],
                    ["expired", "Expired"],
                    ["signed_out", "Signed out"],
                  ])}
                  ${this._previewToggle("Onboarded", "onboarded")}
                  ${this._previewSegment("Remote access", "remote", [
                    ["connected", "Connected"],
                    ["disabled", "Disabled"],
                  ])}
                  ${this._previewSegment("Backups", "backup", [
                    ["fresh", "Recent"],
                    ["stale", "Old"],
                    ["failed", "Failed"],
                    ["none", "None"],
                  ])}
                  ${this._previewToggle("Alexa linked", "alexa")}
                  ${this._previewToggle("Google linked", "google")}
                  ${this._previewToggle("Cameras (WebRTC)", "webrtc")}
                  ${this._previewToggle("Has webhooks", "webhooks")}
                </div>
              `
            : nothing}
        </div>
  ...
  private _previewSegment(
    label: string,
    field: keyof CloudPreview,
        ...
        <div class="segment">
          ${options.map(
            ([value, text]) => html`
              <ha-button
                size="small"
                appearance=${this._preview[field] === value
                  ? "filled"
                  : "plain"}
                data-field=${field}
                data-value=${value}
  ...
  private _previewToggle(
    ...
    return html`
      <div class="preview-row">
        <span>${label}</span>
        <ha-switch
          .checked=${this._preview[field] as boolean}
          data-field=${field}
          @change=${this._handlePreviewToggle}
        ></ha-switch>
      </div>
    `;
  }

  private _handleSegmentClick(ev: Event) {
    const target = ev.currentTarget as HTMLElement;
    this._setPreview(
      target.dataset.field as keyof CloudPreview,
      target.dataset.value!
    );
  }

  private _handlePreviewToggle(ev: Event) {
  ...
  private _setPreview(field: keyof CloudPreview, value: string | boolean) {
    this._preview = { ...this._preview, [field]: value } as CloudPreview;
  }

  private _togglePreview(ev: Event) {
    this._setPreview("enabled", (ev.target as HaSwitch).checked);
  }

  // Effective-state getters: prefer preview overrides when preview mode is on.

  private get _isPreview(): boolean {
    return this._preview.enabled;
  }

  private get _accountState(): CloudPreview["account"] {
    if (this._isPreview) {
      return this._preview.account;
    }
  ...
  private get _remoteEnabled(): boolean {
    return this._isPreview
      ? this._preview.remote !== "disabled"
      : this.cloudStatus.prefs.remote_enabled;
  }

  private get _alexaLinked(): boolean {
    return this._isPreview
      ? this._preview.alexa
      : this.cloudStatus.alexa_registered;
  }

  private get _googleLinked(): boolean {
    return this._isPreview
      ? this._preview.google
      : this.cloudStatus.google_registered;
  }

  private get _webrtcEnabled(): boolean {
    return this._isPreview
      ? this._preview.webrtc
      : this.cloudStatus.prefs.cloud_ice_servers_enabled;
  }

  private get _webhookCount(): number {
    if (this._isPreview) {
      return this._preview.webhooks ? 3 : 0;
    }
  ...
  // Health of the automatic cloud backup, used to colour the status dot:
  // success (green), failed (red, the last attempt did not complete), or
  // old (amber, the schedule says a backup is overdue).
  private get _backupHealth(): "success" | "failed" | "old" | "none" {
    if (this._isPreview) {
      switch (this._preview.backup) {
        case "failed":
          return "failed";
        case "stale":
          return "old";
    ...
    const cfg = this._backupConfig;
    const completed = cfg?.last_completed_automatic_backup
      ? new Date(cfg.last_completed_automatic_backup).getTime()
      : 0;
    const attempted = cfg?.last_attempted_automatic_backup
      ? new Date(cfg.last_attempted_automatic_backup).getTime()
      : 0;
    if (!completed && !attempted) {
      return "none";
    }
    // The most recent attempt is newer than the last success, so it failed.
    if (attempted > completed) {
      return "failed";
    }
    // The next scheduled backup is in the past, so it is overdue.
    const next = cfg?.next_automatic_backup
      ? new Date(cfg.next_automatic_backup).getTime()
      : 0;
  ...
  private get _lastBackupRelative(): string | undefined {
    if (this._isPreview) {
      switch (this._preview.backup) {
        case "stale":
          return "5 days ago";
    ...
    const last = this._backupConfig?.last_completed_automatic_backup;
    return last
  ...
  private _renderBackupStatus(): TemplateResult | string {
    const health = this._backupHealth;
    if (health === "none") {
    ...
    const text =
      health === "failed"
        ? "Backup failed"
        : `Last backup ${this._lastBackupRelative ?? ""}`;
    return html`<span class="status-line"
      ><span class="status-dot ${dot}"></span>${text}</span
    >`;
  }

  private get _email(): string {
    return this._isPreview ? "you@example.com" : this.cloudStatus.email;
  }

  // Hide everything before the @, like the obfuscated remote access URL.
  private get _maskedEmail(): string {
    const email = this._email;
    const at = email.indexOf("@");
    return at > 0 ? `${"•".repeat(at)}${email.slice(at)}` : email;
  }

  private get _renewalDate(): string {
    if (this._isPreview) {
      return this._preview.account === "expired"
        ? "1 June 2026"
        : this._preview.account === "trial"
          ? "27 June 2026"
          : "24 June 2026";
    }
    return this._subscription?.plan_renewal_date
      ? formatDate(
          new Date(this._subscription.plan_renewal_date * 1000),
          this.hass.locale,
          this.hass.config
        )
      : this.hass.localize(
    ...
    return [
      ...
      css`
        ...
        .section-title {
          font-size: var(--ha-font-size-xl);
          font-weight: var(--ha-font-weight-normal);
          margin: var(--ha-space-2) 0 calc(-1 * var(--ha-space-2));
        }
        ha-alert {
          display: block;
